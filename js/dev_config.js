$(document).ready(function(){
		...
		$.ajax({
		  ...
		  success: function(response){
		    ...
		    if (response == 1) {			    
			    ...
			    $('.alert_dev').html('<p class="alert alert-success">Một thiết bị mới được thêm thành công</p>');
	...
	$(document).on('click', '.dev_uid_up', function(){
		...
		bootbox.confirm("Bạn thật sự muốn cập nhật token của thiết bị này?", function(result) {
			if(result){
			     // AJAX Request
			     $.ajax({
			       url: 'dev_config.php',
			       ...
			       success: function(response){
			        ...
			        if(response == 1){			    
				    	$('.alert_dev').fadeIn(500);
			        	$('.alert_dev').html('<p class="alert alert-success">Token của thiết bị này được cập nhật thành công!!!</p>');
	...
	$(document).on('click', '.dev_del', function(){
		...
		bootbox.confirm("Bạn thật sự muốn xóa thiết bị này?", function(result) {
		if(result){
		     // AJAX Request
		     $.ajax({
		       url: 'dev_config.php',
		         ...
		         // Removing row from HTML Table
		          if(response == 1){
		            ...
		            setTimeout(function () {
		              ...
		              bootbox.alert('Thiết bị chưa được xóa.');
		          }
		       }
		     });
		   }
	...
	$(document).on('click', '.mode_sel', function(){
		...
		bootbox.confirm("Bạn thật sự muốn thay đổi cấu hình?", function(result) {
			if(result){
			     // AJAX Request
			     $.ajax({
			       url: 'dev_config.php',
			       ...
			       success: function(response){
			          	...
			          	if(response == 1){
			          		bootbox.alert('<p class="alert alert-success">Đã thay đổi cấu hình thành công</p>');
			              	$(el).closest('tr').css('background','#5cb85c');
					        $(el).closest('tr').fadeOut(300,function(){
					              $(this).show();
					        });
