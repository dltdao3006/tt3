$(document).ready(function(){
    ...
    $.ajax({
      ...
      success: function(response){
        ...
        if (response == 1) {
          ...
          $('#dev_sel').val('0');
          $('.alert_user').fadeIn(500);
          $('.alert_user').html('<p class="alert alert-success">Nhân viên này được cập nhật thành công!</p>');
        }
        else{
          $('.alert_user').fadeIn(500);
          $('.alert_user').html('<p class="alert alert-danger">'+ response + '</p>');
  ...
  $(document).on('click', '.user_rmo', function(){
    ...
    bootbox.confirm("Bạn có thật sự muốn xóa nhân viên này?", function(result) {
      if(result){
        $.ajax({
          url: 'manage_users_conf.php',
          type: 'POST',
